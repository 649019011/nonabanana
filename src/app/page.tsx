  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        if (imgs.length === 0) {
          // Try to be resilient if backend can't extract images yet: search raw payload
          const tryUrls: string[] = [];
          const content = data?.raw?.choices?.[0]?.message?.content;
          if (Array.isArray(content)) {
            for (const item of content) {
              if (item?.image_url?.url) tryUrls.push(item.image_url.url);
              else if (typeof item?.url === 'string') tryUrls.push(item.url);
              else if (typeof item?.image === 'string') tryUrls.push(item.image);
            }
          }
          setGeneratedImages(tryUrls);
        } else {
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <Card className="p-6 bg-gradient-to-br from-yellow-50 to-orange-50 border-orange-200">
              ...
              <div className="space-y-4">
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center cursor-pointer hover:border-orange-400 transition-colors"
                  ...
                  {uploadedImage ? (
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-green-600 font-medium">✓ 图片已上传</p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {generatedImages.map((url, idx) => (
                        <div key={idx} className="bg-white p-2 rounded-lg border border-gray-200">
                          <img src={url} alt={`Generated ${idx + 1}`} className="w-full h-auto rounded" />
                        </div>
                      ))}
                    </div>
                    <Button
